document.addEventListener('DOMContentLoaded', () => {
    function initFooterFeatures() {
        ...
        if (toggleBtn && footer) {
            toggleBtn.addEventListener('click', () => {
                footer.style.backgroundColor = isDefaultTheme ? '#1e3d58' : '#4a90e2';
    ...
    // Ensure HTMLInclude finishes before running
    if (window.HTMLInclude) {
        HTMLInclude();
        setTimeout(initFooterFeatures, 500); // delay allows content to load
    } else {
        initFooterFeatures();
    }
});